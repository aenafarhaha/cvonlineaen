...
const ContactForm = () => {
  ...
  return (
    <section className="p-4 bg-blue-100 shadow-lg rounded-lg max-w-6xl mx-auto">
      <h2 className="text-md font-bold mb-4 text-center text-black-600">Contact Form</h2>
      {submitted && <p className="text-green-600 text-center">Terima kasih atas komentarnya!</p>}
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label className="block text-black-700 text-sm mb-1" htmlFor="name">Nama</label>
          <input 
            type="text" 
            id="name" 
            value={name} 
        ...
        <div>
          <label className="block text-black-700 text-sm mb-1" htmlFor="comment">Komentar</label>
          <textarea 
            id="comment" 
            value={comment} 
        ...
        <div>
          <label className="block text-black-700 text-sm mb-1">Rating</label>
          <div className="flex">
            {[1, 2, 3, 4, 5].map((star) => (
              <span 
                key={star} 
                className={`cursor-pointer text-${rating >= star ? 'yellow-600' : 'grey-100'} text-2xl`}
      ...
      <div className=" mt-6">
        <h3 className="text-md font-semibold text-center text-black-600">Komentar yang Dikirim</h3>
